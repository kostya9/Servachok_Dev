...
class Test_case_MapGenerator(unittest.TestCase):
    ...
    def test_check_start_position_equal_radius_generated(self): #Точность теста около 1 процента +-
        ...
        for i in range(len(self.mg._MapGenerator__planets)-1):
            self.assertEqual(self.mg._MapGenerator__planets[i+1].coords.radius_calculation()-temp<(temp/100),True,"radiuses arent the same")
    ...
    def test_check_start_position_player_to_player_distance(self): #Точность теста около 1 процента +-
        ...
        temp=self.mg._MapGenerator__planets[1].coords.calc_distance(self.mg._MapGenerator__planets[len(players_ids)].coords)
        for i in range(len(players_ids)-1):
            self.assertEqual(self.mg._MapGenerator__planets[i+1].coords.calc_distance(self.mg._MapGenerator__planets[i+2].coords)-temp<(temp/100),True,"wrong distance to planets")
