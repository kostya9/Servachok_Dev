...
from utils import Singleton, MaxPriorityQueue, id_generator, StoppedThread, Config

CFG = Config()

MAX_CLIENT_COUNT = int(CFG['Server']['max_client_count'])
DEFAULT_SERVER_ADDRESS = (CFG['Server']['ip'], int(CFG['Server']['port']))
SELECT_TIMEOUT = int(CFG['Server']['select_timeout'])
...
class Server(metaclass=Singleton):
    def __init__(self, address=DEFAULT_SERVER_ADDRESS, max_client_count=MAX_CLIENT_COUNT):
        self.server = create_tcp_server(address, max_client_count)
        self.started = True
        self.clients = []
    ...
    def receiver(self):
        readable, *_ = select.select([self.server, *self.clients], [], [], SELECT_TIMEOUT)
        ...
        if not self.__handler_queue.empty():
            ...
            if event['name'] == "ready":
                ...
                if ready and len(self.players) > 1:
                    gen = MapGenerator()
                    __map = gen.run([player['id'] for player in self.players.values()])
                    gen.display()
                    game_map = [planet.get_dict() for planet in __map]
